...
> Tomato Focus is a Pomodoro timer. The foreground service keeps a
...
Everything in `assets/` was generated by `tool/generate_assets.py` — there are
no third-party assets and no licences to track. Regenerate with:
...
| `icon_adaptive_foreground.png` | Adaptive icon foreground, art kept inside the 66% safe zone. Also used as the monochrome layer. |
| `brand_lockup_light/dark.png` | The tomato with **"Productivity"** underneath — the in-app lockup, used on splash and About. This is where the word lives; baking it into a 48dp launcher icon would make both illegible. |
...
1. Create a keystore and `android/key.properties` (git-ignored, template at
   ...
   ```properties
   storeFile=../keystore/tomato-focus.jks
   storePassword=...
   keyAlias=tomato
   keyPassword=...
   ```
...
- [ ] **Privacy policy URL** set in Play Console *and* reachable. Replace the
      placeholder in `lib/core/constants.dart`
      (`AppConstants.privacyPolicyUrl`) — it is linked from Settings → About,
      which Play expects for an app requesting notification and alarm access.
      `PRIVACY_POLICY.md` in this repo is a ready-to-host draft.
...
- [ ] **Notification Policy Access (DND control) declaration.**
      ...
      > Tomato Focus is a Pomodoro timer. With the user's explicit opt-in, it
